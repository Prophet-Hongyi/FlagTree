...
# Kernel 3: intra-node atomic barrier using tle.remote + atomic_cas
@triton.jit
def atomic_barrier_kernel(
    local_flag_ptr,
    dev_mem_ptr,
    LOCAL_RANK: tl.constexpr,
    WORLD_SIZE: tl.constexpr,
    ...
    if peer == LOCAL_RANK:
        remote_flag = local_flag_ptr + LOCAL_RANK
    else:
        remote_flag = tle.remote(
            dev_mem_ptr,
            space="device",
            dtype=tl.int32,
            shard_id=peer,
    ...
    while tl.atomic_cas(remote_flag, 0, 1, sem="release", scope="sys") != 0:
        pass

    local_slot = local_flag_ptr + peer
    while tl.atomic_cas(local_slot, 1, 0, sem="acquire", scope="sys") != 1:
        pass
...
# TLE reduce-scatter host wrapper (optimized scatter -> atomic barrier -> TMA reduce)
def tle_reduce_scatter_atomic_barrier(
    input_tensor,
    scatter_buf,
    scatter_dev_mem_ptr,
    output,
...
def main():
    ...
    print(f"[Rank {rank}/{world_size}] Starting TLE reduce-scatter atomic barrier (4096, 2048)")
    ...
    # Correctness check.
    tle_reduce_scatter_atomic_barrier(
        input_tensor,
        scatter_buf,
        scatter_dev_mem_ptr,
        output,
