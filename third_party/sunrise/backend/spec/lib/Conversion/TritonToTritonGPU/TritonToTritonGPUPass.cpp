...
static bool shouldClampSunriseNumWarpsForSmallReduce(
  ...
  bool hasDot = false;
  bool hasSmallRank1Reduce = false;
  mod.walk([&](Operation *op) {
    if (isa<triton::DotOp>(op)) {
      hasDot = true;
      return WalkResult::interrupt();
    }
    auto reduce = dyn_cast<triton::ReduceOp>(op);
    if (!reduce)
    ...
    int64_t reduceWidth = operandTy.getShape()[0];
    if (reduceWidth > 0 && reduceWidth <= 128) {
      hasSmallRank1Reduce = true;
      // 1 warp for reduceWidth <= 64, 2 warps for reduceWidth > 64 and <= 128
      clampedNumWarps = reduceWidth <= 64 ? 1 : 2;
    }
    return WalkResult::advance();
  });
  if (hasSmallRank1Reduce && !hasDot)
    numWarps = clampedNumWarps;
  return hasSmallRank1Reduce && !hasDot;
}
