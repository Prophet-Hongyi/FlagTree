...
def _effective_profitability_policy() -> tuple[int, int, int, int, int, int, int, int, int, int]:
    ...
    return (
        ...
        int(getattr(hcu_knob, "rlc_profitability_min_adjusted_saved_cost_per_tensor_op", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_phase3_saved_cost_multiplier", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_max_external_use_edges", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_min_removed_convert_density_per_1024_proposal_values", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_low_density_global_writeback_min_math_ops", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_low_density_output_heavy_min_compute_ops", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_low_density_zero_load_min_arithmetic_ops", 0) or 0),
        int(getattr(hcu_knob, "rlc_profitability_low_density_loop_resident_min_saved_cost", 0) or 0),
    )
...
def _apply_hcu_rlc_policy(mod) -> None:
    ...
    if knobs.hcu.rlc_allow_atomic_writeback_order_change:
        mod.set_attr("ttg.rlc-allow-atomic-writeback-order-change",
    ...
    if not enabled:
        return
    mod.set_attr("ttg.rlc-profitability-policy-enabled", builder.get_int32_attr(1))
    if launch_count > 0:
        mod.set_attr("ttg.rlc-product-launch-count", builder.get_int32_attr(launch_count))
    if min_score > 0:
    ...
    if min_removed_convert_density > 0:
        mod.set_attr("ttg.rlc-profitability-min-removed-convert-density-per-1024-proposal-values",
                     builder.get_int32_attr(min_removed_convert_density))
    if low_density_global_writeback_min_math_ops > 0:
        mod.set_attr("ttg.rlc-profitability-low-density-global-writeback-min-math-ops",
                     builder.get_int32_attr(low_density_global_writeback_min_math_ops))
    if low_density_output_heavy_min_compute_ops > 0:
        mod.set_attr("ttg.rlc-profitability-low-density-output-heavy-min-compute-ops",
                     builder.get_int32_attr(low_density_output_heavy_min_compute_ops))
    if low_density_zero_load_min_arithmetic_ops > 0:
        mod.set_attr("ttg.rlc-profitability-low-density-zero-load-min-arithmetic-ops",
