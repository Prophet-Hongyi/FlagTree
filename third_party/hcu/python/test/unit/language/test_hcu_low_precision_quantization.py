...
_GFX936_INT8_MMAC_LLIR = "llvm.hcu.mmac.i32.16x16x32.i8"
_GFX936_INT8_MMAC_ASM = "v_mmac_i32_16x16x32_i8"
_GFX936_FP16_MMAC_LLIR = "llvm.hcu.mmac.f32.16x16x16.f16"
_GFX936_FP16_MMAC_ASM = "v_mmac_f32_16x16x16_f16"
...
def test_gfx936_native_int8_dot_has_explicit_int8_output_epilogue():
    _require_gfx936()
    ...
    assert _GFX936_INT8_MMAC_LLIR in compiled.asm["llir"]
    assert _GFX936_FP16_MMAC_LLIR not in compiled.asm["llir"]
    assert _GFX936_INT8_MMAC_ASM in compiled.asm["amdgcn"]
    assert _GFX936_FP16_MMAC_ASM not in compiled.asm["amdgcn"]
    assert compiled.asm["hsaco"]


def test_gfx936_int8_storage_fp16_compute_int8_output():
    _require_gfx936()
    ...
    torch.testing.assert_close(output.cpu(), expected, rtol=0, atol=0)
    assert _GFX936_FP16_MMAC_LLIR in compiled.asm["llir"]
    assert _GFX936_INT8_MMAC_LLIR not in compiled.asm["llir"]
    assert _GFX936_FP16_MMAC_ASM in compiled.asm["amdgcn"]
    assert _GFX936_INT8_MMAC_ASM not in compiled.asm["amdgcn"]
    assert compiled.asm["hsaco"]


def test_gfx936_uint8_storage_fp16_compute_uint8_output():
    _require_gfx936()
