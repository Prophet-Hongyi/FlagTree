        ...
        if a_is_scalar != b_is_scalar:
            ...
            if scalar_ty.kind().value <= tensor_ty.kind().value:
                if div_or_mod and (tensor_ty in (tl.float16, tl.bfloat16)):
                    return tl.float32
                return tensor_ty
            # For same-kind scalar/tensor pairs (e.g. fp32 scalar + bf16
            # tensor), keep applying generic promotion rules so fp32 is
            # preserved instead of being prematurely demoted to bf16.
...
# ===----------------------------------------------------------------------===//
    ...
    def _assert_dot_dtypes_valid(self, lhs_dtype: tl.dtype, rhs_dtype: tl.dtype) -> None:
        supported_fp8 = set(getattr(self.builder.options, "supported_fp8_dtypes", ()))
        deprecated_fp8 = set(getattr(self.builder.options, "deprecated_fp8_dot_operand_dtypes", ()))
        ...
        if lhs_dtype.is_fp8() or rhs_dtype.is_fp8():
            ...
            for dtype in (lhs_dtype, rhs_dtype):
                ...
                if supported_fp8 and dtype_name not in supported_fp8 and dtype_name not in deprecated_fp8:
                    raise ValueError(f"{dtype_name} is not supported in this architecture for dot on this target. "
                                     f"Supported fp8 dtypes: {sorted(supported_fp8)}")
            return

        allowed = (tl.int8, tl.uint8, tl.float16, tl.bfloat16, tl.float32, tl.float64)
