...
def _uint8_inputs():
    lhs = ((torch.arange(32 * 64) * 13 + 17) % 256).reshape(32, 64).to(torch.uint8)
    rhs = ((torch.arange(64 * 32) * 29 + 5) % 256).reshape(64, 32).to(torch.uint8)
    return lhs.to("musa"), rhs.to("musa")
...
def test_ph1_uint8_storage_bf16_compute_uint8_output():
    _require_ph1()

    lhs, rhs = _uint8_inputs()
    output = torch.empty((32, 32), dtype=torch.uint8, device="musa")
