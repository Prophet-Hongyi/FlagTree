...
# ===----------------------------------------------------------------------===//
    ...
    def _load_legacy(self, ptr, mask, other, boundary_check, padding, cache, eviction, is_volatile, smem_hints):
        ...
        # Build IR
        if mask is None:
            ret = self.tensor(self.builder.create_load(ptr.handle, cache, eviction, is_volatile, smem_hints), dst_ty)
        else:
            ret = self.tensor(
                self.builder.create_masked_load(ptr.handle, mask.handle, other.handle if other else None, cache,
                                                eviction, is_volatile, smem_hints), dst_ty)
        if is_bool:
