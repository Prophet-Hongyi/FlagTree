"""Enflame RLC option validation shared by GCU300/GCU400 pipelines."""

from collections.abc import Mapping


SUPPORTED_RLC_ARCHES = frozenset({"gcu300", "gcu400", "gcu410"})
...
def validate_rlc_phase_mask(value):
    ...
    if mask < 0 or mask & ~0xF:
        raise ValueError(f"Enflame RLC phase mask must be in [0, 15], got {value!r}")
    return mask


def validate_rlc_arch(arch, enhance):
    if enhance and arch not in SUPPORTED_RLC_ARCHES:
        supported = ", ".join(sorted(SUPPORTED_RLC_ARCHES))
        raise ValueError(f"Enflame enhanced RLC supports only {supported}; got {arch!r}")


def rlc_pass_options(enhance, phase_mask):
    mask = validate_rlc_phase_mask(phase_mask)
    enabled = "true" if bool(enhance) else "false"
    return f"enable-rlc-enhance={enabled} rlc-phase-mask={mask}"
...
def rlc_policy_signature(config):
    ...
    for name, _ in RLC_POLICY_FIELDS:
        ...
        if value < 0:
            raise ValueError(f"Enflame RLC policy override {name} must be non-negative, got {value}")
