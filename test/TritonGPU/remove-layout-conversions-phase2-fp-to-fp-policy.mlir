...
// RUN: triton-opt %s -split-input-file -tritongpu-remove-layout-conversions="enable-rlc-enhance=true rlc-phase-mask=8" | FileCheck %s --check-prefixes=VECTORIZED,UNSUPPORTED
...
// RUN: triton-opt %s -split-input-file -tritongpu-remove-layout-conversions="enable-rlc-enhance=true rlc-phase-mask=13" | FileCheck %s --check-prefixes=VECTORIZED,UNSUPPORTED
...
// TRACE3-DAG: FLAGTREE_RLC_TRACE phase=3 outcome=preserve reason=fp-to-fp-vector-width
// TRACE3-DAG: FLAGTREE_RLC_TRACE phase=3 outcome=preserve reason=fp-to-fp-contiguity-boundary
